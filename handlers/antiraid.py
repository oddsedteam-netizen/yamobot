...
def antiraid_kb() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔢 Количество заходов", callback_data="antiraid_threshold")],
        [InlineKeyboardButton(text="🔗 Удаление ссылок", callback_data="antiraid_links_ask")],
        [InlineKeyboardButton(text="👥 Удаление зашедших", callback_data="antiraid_members_ask")],
...
async def cb_antiraid_threshold(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="❌ Отмена", callback_data="antiraid")]
    ])
    if callback.message:
...
async def fsm_antiraid_threshold(message: Message, state: FSMContext) -> None:
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⬅️ К настройкам антирейда", callback_data="antiraid")]
    ])
...
async def cb_antiraid_links_ask(callback: CallbackQuery) -> None:
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Да, удалять ссылки", callback_data="antiraid_links_set_1")],
        [InlineKeyboardButton(text="❌ Нет", callback_data="antiraid_links_set_0")],
        [InlineKeyboardButton(text="⬅️ Назад", callback_data="antiraid")],
    ])
    if callback.message:
        await try_edit_answer(
            callback.message,
...
async def cb_antiraid_members_ask(callback: CallbackQuery) -> None:
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Да, удалять зашедших", callback_data="antiraid_members_set_1")],
        [InlineKeyboardButton(text="❌ Нет", callback_data="antiraid_members_set_0")],
        [InlineKeyboardButton(text="⬅️ Назад", callback_data="antiraid")],
    ])
    if callback.message:
        await try_edit_answer(
