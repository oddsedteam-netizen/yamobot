...
FAQ_TEXT = (
    ...
    "🛡 <b>Модерация в чате админов</b>\n"
    "Команды работают в привязанном чате админов — по реплаю на любое "
    "сообщение или с указанием юзера (ID / @имя). Команды бан/мут/предов могут "
    "использовать владелец и назначенные модераторы:\n"
    "• <code>/бан 30м</code> — бан на время (или /бан без времени = навсегда).\n"
    "• <code>/разбан</code> — снять бан (реплай или указать юзера).\n"
    "• <code>/мут 1ч</code> — запрет писать на время.\n"
    "• <code>/размут</code> — снять мут.\n"
    "• <code>/пред</code> / <code>/варн</code> — выдать пред (до порога из /предред).\n"
    "• <code>/снять варн</code> — снять преды.\n"
    "• <code>/предред 5 мут 1ч</code> — порог предов и наказание (бан/мут + время).\n"
    "• <code>/стата неделя</code> — статистика админов (доступна всем в чате).\n"
    "👑 Только владелец:\n"
    "• <code>/модер @юзер</code> — выдать права модератора чата.\n"
    "• <code>/разжаловать @юзер</code> — снять права модератора.\n"
    "• <code>/модеры</code> — список модераторов чата.\n\n"
    "Время: <code>30м</code>, <code>2ч</code>, <code>3д</code>, <code>1нед</code>.\n\n"
